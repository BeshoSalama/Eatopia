...
def detect_food_label_with_ollama(image_path):
    try:
        from engine.ollama_vision import detect_food_with_ollama

        label = str(detect_food_with_ollama(image_path) or "").strip()
        if label.lower() in {"", "unknown", "unknown_food", "egyptian_food"}:
            return ""
...
def fallback_scan(image_path):
    ...
    vision_label = detect_food_label_with_ollama(image_path)
    if vision_label:
        name = humanize_food_name(vision_label)
...
def inverse_cooking_scan(image_path):
    ...
    vision_label = detect_food_label_with_ollama(image_path)
    food_name = humanize_food_name(vision_label) if vision_label else result.get("title") or "Scanned Meal"
    ...
    return {
        ...
        **nutrition,
        ...
        "source": "ollama+inverse-cooking" if vision_label else "inverse-cooking",
