...
public class AuthServiceTests
{
    ...
    [Fact]
    public async Task RequestPasswordResetAsync_ThrowsWhenEmailCannotBeSent()
    {
        await using var database = await TestDatabase.CreateAsync();
        var user = await database.AddUserAsync("reset@eatopia.local");
        var service = TestServices.AuthService(database);

        var exception = await Assert.ThrowsAsync<ApiException>(() =>
            service.RequestPasswordResetAsync(new ForgotPasswordRequestDto
            {
                Email = user.Email
            }));

        Assert.Equal(500, exception.StatusCode);
        Assert.Equal("EMAIL_SEND_FAILED", exception.Code);

        var activeCodeCount = await database.Context.PasswordResetCodes.CountAsync(x =>
            x.UserId == user.Id &&
            !x.IsUsed &&
            x.ExpiresAt > DateTime.UtcNow);

        Assert.Equal(1, activeCodeCount);
